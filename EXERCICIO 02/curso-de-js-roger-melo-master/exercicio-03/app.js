...
*/
  const foodsInfo = `Até aqui, o array "\nbrazilianFoods\n" possui ${brazilianFoods.length} itens: ${brazilianFoods[0]},${brazilianFoods[1]} e ${brazilianFoods[2]}`
...
*/
  const maleDogNmames = ['Chico','Zeca']
  const femaleDogNames = ['Lilica','Matilda']
  let dogNames = maleDogNmames.concat(femaleDogNames)
...
*/
   dogNames.push('Nina')
   const initials = `${dogNames[0].slice(0,2)}${dogNames[1].slice(0,2)}${dogNames[2].slice(0,2)}${dogNames[3].slice(0,2)}`
...
*/
   const dessert = `${dogNames[0].slice(3)}${dogNames[1].slice(2)}${dogNames[2].replace('L','d')}${dogNames[4].slice(6)}`
   //console.log(dessert)
  //Incompleto refazer
