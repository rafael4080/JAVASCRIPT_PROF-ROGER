...
*/
const fruits = ['morango', 'pera', 'mamão']
fruitsprime = fruits.includes('abacaxi')
fruitstwo = fruits.includes('pera')
if(fruitsprime){
    //console.log('abacaxi existe no array fruits')
}else if(fruitstwo){ 
  // console.log('pera exite no array fruits')
}else{
...
for(let i = 0;i < crazyArray.length;i++){
  const isItemNumber =typeof crazyArray[i] === 'number'
  const isItemBoolean = typeof crazyArray[i] === 'boolean'
    if(isItemNumber){
        numberMoute = numberMoute + 1
    }else if(isItemBoolean ){
        booleanMoute = booleanMoute + 1
    }else{
       stringMount = stringMount +1
    }
}
console.log(`O crazyArray tem ${booleanMoute} booleans, ${numberMoute} números e ${stringMountb} strings.`)
/*
  06
